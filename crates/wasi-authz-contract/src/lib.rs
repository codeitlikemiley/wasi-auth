...
pub enum ContractError {
    ...
    /// A decision attempted to control client-side caching.
    #[error("decision caching is unsupported")]
    CachingUnsupported,
    /// A decision contained an unrecognized context member.
    #[error("unsupported AuthZEN decision context member: {0}")]
    UnsupportedDecisionContext(String),
}
...
impl DecisionResponseV1 {
    ...
    /// Decodes a successful AuthZEN response and rejects unsupported context.
    ///
    /// # Errors
    ///
    /// Returns a contract error for malformed JSON, obligations, advice,
    /// caching instructions, or unrecognized decision context.
    pub fn from_json_slice(input: &[u8]) -> Result<Self, ContractError> {
...
fn parse_decision_context(
    ...
    for reserved in ["obligations", "obligation", "advices", "advice"] {
        if context.contains_key(reserved) {
            return Err(ContractError::ObligationsUnsupported);
        }
    }
    for reserved in ["cache", "cache_control", "max_age"] {
        if context.contains_key(reserved) {
            return Err(ContractError::CachingUnsupported);
        }
    }
    let profile = context.remove(PROFILE_PROPERTY).ok_or_else(|| {
        let member = context
            .keys()
            .next()
            .cloned()
            .unwrap_or_else(|| "context".to_owned());
        ContractError::UnsupportedDecisionContext(member)
    })?;
    if let Some(member) = context.keys().next() {
        return Err(ContractError::UnsupportedDecisionContext(member.clone()));
    }
    let Some(profile_object) = profile.as_object() else {
        return Err(ContractError::UnsupportedDecisionContext(
            PROFILE_PROPERTY.to_owned(),
        ));
    };
    for reserved in ["obligations", "obligation", "advices", "advice"] {
        if profile_object.contains_key(reserved) {
            return Err(ContractError::ObligationsUnsupported);
        }
    }
    for reserved in ["cache", "cache_control", "max_age"] {
...
mod tests {
    ...
    #[test]
    fn decision_obligation_is_rejected_fail_closed() {
        let input = br#"{"decision":true,"context":{"obligations":[]}}"#;
    ...
    #[test]
    fn decision_cache_instruction_is_rejected_fail_closed() {
        let input = br#"{"decision":true,"context":{"wasi_authz":{"cache":{"max_age":30}}}}"#;

        let result = DecisionResponseV1::from_json_slice(input);

        assert!(matches!(result, Err(ContractError::CachingUnsupported)));
    }

    #[test]
    fn unknown_decision_context_is_rejected_fail_closed() {
        let input = br#"{"decision":true,"context":{"reason":"allowed"}}"#;

        let result = DecisionResponseV1::from_json_slice(input);

        assert!(matches!(
            result,
            Err(ContractError::UnsupportedDecisionContext(member)) if member == "reason"
        ));
    }
